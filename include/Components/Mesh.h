...
private:
	void VertexBuffers(float* vertices, unsigned int* indices, unsigned int verticesCount, unsigned int indicesCount)
	{
		...
		glBufferData(GL_ARRAY_BUFFER, sizeof(unsigned int) * verticesCount, vertices, GL_STATIC_DRAW);

		glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, sizeof(unsigned int) * 5, 0);
		glEnableVertexAttribArray(0);
		glVertexAttribPointer(1, 2, GL_FLOAT, GL_FALSE, sizeof(unsigned int) * 5, (void*)(sizeof(vertices[0]) * 3));
